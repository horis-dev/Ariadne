...
def model_test():
    from basic.position import ApiPosition, ResponsePosition, FilePosition, TablePosition
    api_positions = [ApiPosition(b"API1", "name"), ApiPosition(b"API1", "project_name"),
                     ApiPosition(b"API2", "csrf_token"), ApiPosition(b"API3", "name")]

    init_words = [
        FuzzWord("pyh", ApiPosition(b"API1", "name")),
        FuzzWord("webfuzz", ApiPosition(b"API1", "project_name")),
        FuzzWord("csrf_token_1234", ApiPosition(b"API2", "csrf_token")),
        FuzzWord("csrf_token_1234", ResponsePosition("HTTP", "csrf_token", b"API1")),
        FuzzWord("pyh", TablePosition("users", "user_name")),
        FuzzWord("webfuzz", TablePosition("projects", "project_name")),
        FuzzWord(b"Hello, world!", FilePosition(True, "/tmp")),
    ]
    target_words = [
        FuzzWord("hacker", TablePosition("users", "user_name")),
        FuzzWord("bad_project", TablePosition("projects", "project_name")),
        FuzzWord(b"Attack File", FilePosition(True, "/tmp")),
    ]

    local_word_lib = {
        0: {
            ResponsePosition("HTTP", "csrf_token", b"API1"): ["csrf_token_5678"],
        }
    }

    print(f"api positions: {len(api_positions)}")
    data = KeyDependencyModelData(all_api_positions=api_positions, init_word_lib=WordLib(init_words),
                                  target_word_lib=WordLib(target_words))
    model = KeyDependencyModel(data)
    """
    count = 0
    print("\nDependency graph:")  
    for pos, deps in model.dependency_graph.items():  
        count += 1   
        print(f"Api_Position_{count}: {pos}")  
        for dep_pos, prob in deps.items():  
            print(f"  -> {dep_pos} with probability {prob}")
    """
    # Test mutation for 'name' type
    value = model.mutate_value(ApiPosition(b"API1", "name"), global_word_lib=WordLib([]), local_word_lib=local_word_lib)
    print(f"name Mutated values: {value}\n")
    # Test mutation for 'csrf_token' type
    value = model.mutate_value(ApiPosition(b"API2", "csrf_token"), global_word_lib=WordLib([]),
                               local_word_lib=local_word_lib)
    print(f"csrf_token Mutated values: {value}")
